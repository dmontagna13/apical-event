...
5. **Atomic action pane population.** The action pane cannot be partially populated. It populates in full when the Moderator API call returns — all cards and quizzes emitted by a single Moderator turn arrive together as a single state update. The user will never see an action pane that is "still loading" more cards from an in-progress Moderator call. This is not a design choice; it is a structural consequence of the Moderator being a single synchronous API call whose tool calls are parsed after the full response is received.
...
The backend triggers runner.start_session asynchronously and returns immediately. start_session assembles init prompts for all roles directly from the packet — this is a deterministic template operation; no LLM is involved. It then dispatches API calls for all roles in parallel: the Moderator receives its full system prompt and an opening context message (session objective + agenda); each background agent receives its role system prompt, inlined pre-reads, constraints, and a brief orientation message. The session transitions to ACTIVE with substate INIT_DISPATCH. The UI switches from the Roll Call screen to the three-pane workbench and shows a loading state in the center pane while init calls are in flight. Agent responses stream into the center pane as they arrive. The action pane remains empty until the Moderator has processed the bundle.
...
The deliberation loop has two distinct phases: a session-start path (runs once, triggered by roll call confirmation) and the deliberation loop (repeating cycle).

```
SESSION START (triggered by roll call confirmation)
        │
        ▼
  INIT_DISPATCH  ←— runner.start_session only. NOT a LangGraph node.
  Deterministic prompts assembled from packet.
  All roles dispatched in parallel (Moderator + all background agents).
  Runs exactly once per session. Never re-entered.
        │
        ▼
AGENT_AGGREGATION ◄──────────────────────────────────────────┐
(cycle 1: bundle background agent init responses;             │
 cycle 2+: bundle deliberation responses.                     │
 Either way: deliver bundle to Moderator history,             │
 transition to MODERATOR_TURN)                                │
        │                                                     │
        ▼                                                     │
  MODERATOR_TURN                                              │
  (Moderator processes bundle, emits                          │
   conversational text + tool calls)                          │
        │                                                     │
        ▼                                                     │
   HUMAN_GATE                                                 │
   (User reviews action cards,                                │
    answers quizzes, chats with Moderator.                    │
    First human interaction point in the session.)            │
        │                                                     │
        ▼                                                     │
  AGENT_DISPATCH                                              │
  (Human-approved prompts sent to                             │
   background agents in parallel.                             │
   Deliberation dispatch only — never init.)                  │
        │                                                     │
        └─────────────────────────────────────────────────────┘
```
...
INIT_DISPATCH is a runner function, not a LangGraph node. It cannot be reached by graph routing.
MODERATOR_TURN is only entered after AGENT_AGGREGATION has produced a bundle. It is never the session entry point.
AGENT_DISPATCH (the graph node) handles deliberation dispatch only — approved action cards from HUMAN_GATE. Init dispatch is handled exclusively by runner.start_session.
The LangGraph graph's entry point is AGENT_AGGREGATION.
HUMAN_GATE is the first and only point where the human can interact with the session. It is not reached until the Moderator has processed the first bundle.

#### 4.4.2 MODERATOR_TURN



**Entry condition:** `MODERATOR_TURN` is only entered from `AGENT_AGGREGATION`. A bundle must exist in state before this node runs. If called without a bundle, the engine raises `EngineStateError` — this indicates a bug in graph routing, not a recoverable condition. On cycle 1, this node receives `bundle_001` (background agent init responses). On cycle 2+, it receives the latest deliberation bundle. LangGraph invokes the Moderator's API endpoint with:
...
**First human interaction.** `HUMAN_GATE` is the first point in the session where the human can interact. By the time this node is reached on cycle 1, the user has already seen: the Moderator's opening message (streamed during init dispatch), all background agents' initial responses (streamed as they arrived), and the Moderator's analysis of those responses including any proposed action cards or quizzes. The human enters `HUMAN_GATE` with full context, not a blank slate.
...
FastAPI sends the approved prompts to the background agents' API endpoints **in parallel**. **Deliberation dispatch only.** This node handles cycle 2+ dispatch of human-approved action cards. Init dispatch — the parallel call to all agents at session start — is handled by `runner.start_session` before the LangGraph graph is entered. Init prompts are assembled deterministically from the packet; they do not transit through `MODERATOR_TURN`, `HUMAN_GATE`, or this node. By the time this node is entered, `approved_cards` is guaranteed non-empty (enforced by `HUMAN_GATE` routing).
